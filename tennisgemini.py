...
- null if data unavailable"""
    ...
    def normalize_to_schema(self, skim_data: dict, focus_data: dict, video_path: str) -> dict:
        ...
        # Match metadata
        match_meta = skim_data.get('match_metadata') or {}
        players_data = skim_data.get('players') or {}
        ...
        # Match result
        match_result = skim_data.get('match_result') or {}
        winner = match_result.get('winner')
        
        # Get sets_won with safe defaults
        sets_won = match_result.get('sets_won') or {}
        p1_sets = sets_won.get('player_1') or 0
        p2_sets = sets_won.get('player_2') or 0
        
        # If winner is None or invalid, determine from sets_won
        if winner not in ['player_1', 'player_2']:
            winner = 'player_1' if p1_sets > p2_sets else 'player_2'
        
        output['match_result'] = {
            'winner': winner,
            'score': match_result.get('score', []),
            'sets_won': {'player_1': p1_sets, 'player_2': p2_sets},
            'games_won': {'player_1': 0, 'player_2': 0},
            'total_points': {'player_1': 0, 'player_2': 0}
        }
        
        # Sets overview
        sets_overview = skim_data.get('sets_overview') or []
        ...
        # Events
        events = skim_data.get('key_events') or {}
        
        # Aces
        for ace in events.get('aces') or []:
        ...
        # Double faults
        for df in events.get('double_faults') or []:
        ...
        # Breaks of serve
        for brk in events.get('breaks_of_serve') or []:
        ...
        # Pressure points
        for pp in events.get('pressure_points') or []:
        ...
        # Statistics
        detailed_stats = focus_data.get('detailed_statistics') or {}
        
        for player_id in ['player_1', 'player_2']:
            player_stats = detailed_stats.get(player_id) or {}
            
            output['statistics'][player_id] = json.loads(json.dumps(PLAYER_STATISTICS_SCHEMA))
            
            # Serve stats
            serve = player_stats.get('serve') or {}
            ...
            # Return stats
            ret = player_stats.get('return') or {}
            ...
            # Points stats
            pts = player_stats.get('points') or {}
            ...
            # Performance stats
            rallies = player_stats.get('rallies') or {}
        ...
        # Pressure analysis
        pressure_analysis = focus_data.get('pressure_analysis') or {}
        
        for player_id in ['player_1', 'player_2']:
            player_pressure = pressure_analysis.get(player_id) or {}
            
            output['pressure_analysis'][player_id] = json.loads(json.dumps(PRESSURE_PERFORMANCE_SCHEMA))
            
            # Break points
            bp = player_pressure.get('break_points') or {}
            ...
            # Crucial points
            cp = player_pressure.get('crucial_points') or {}
            ...
            # Set points
            sp = player_pressure.get('set_points') or {}
            ...
            # Match points
            mp = player_pressure.get('match_points') or {}
            ...
            # Tiebreaks
            tb = player_pressure.get('tiebreaks') or {}
            ...
            # Momentum
            mom = player_pressure.get('momentum') or {}
        ...
        # Key moments
        output['pressure_analysis']['key_moments'] = pressure_analysis.get('key_moments') or []
        ...
        output['analysis_metadata'] = {
            ...
            'segments_analyzed': skim_data.get('focus_segments') or [],
    ...
    def save_normalized_output(self, normalized: dict, base_name: str):
        """Save normalized output as single JSON file."""
    ...
    async def run(self, video_path: str):
        ...
        try:
            ...
            winner_id = normalized['match_result']['winner']
            winner_name = normalized['players'].get(winner_id, {}).get('name', 'Unknown') if winner_id else 'Unknown'
            ...
            logger.info(f"\n📁 Output: outputtennis/{match_id}.json")
