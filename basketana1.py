...
3. Detect the year when the match was played (from scoreboard, jerseys, graphics, or other visual cues)
4. Find ALL player substitutions (when players enter/exit)
5. Detect high-pressure moments (close score + late quarter)
6. Identify ALL unique players (jersey numbers/names)
7. Mark EVERY key event timestamp (shots, fouls, blocks, etc.)
...
CRITICAL: 
- Track EVERY significant play (shots, fouls, blocks, turnovers, etc.) with timestamps.
- Extract the match year from visible elements like scoreboard graphics, jersey styles, or broadcast overlays.
- If the year cannot be determined, use null for match_year.
Return valid JSON only."""
    ...
    def create_focus_prompt(self, skim_data: Dict) -> str:
        ...
        match_year = video_structure.get('match_year', 'Unknown')
...
- Return valid JSON only"""
    ...
    async def pass1_skim_analysis(self, video_file: genai.types.File) -> Dict:
        ...
        logger.info("→ Detecting teams, quarters, year, players, and key moments")
        ...
        for attempt in range(max_retries):
            try:
                ...
                logger.info(f"  - Match Year: {video_structure.get('match_year', 'Unknown')}")
    ...
    def convert_to_csv(self, merged_data: Dict) -> pd.DataFrame:
        ...
        for player in merged_data['players']:
            row = {
                ...
                'match_year': video_structure.get('match_year'),
    ...
    async def run_two_pass_analysis(self, video_path: str):
        ...
        try:
            ...
            match_year = video_structure.get('match_year', '')
            year_suffix = f"_{match_year}" if match_year else ""
            base_filename = f"{team_a}_vs_{team_b}{year_suffix}_{timestamp}"
